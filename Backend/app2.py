...
def rayleigh_mmse(sig, snr_db):
    ...
    r = np.zeros_like(sig, dtype=np.complex64)
    equalized_signal = np.zeros_like(sig, dtype=np.complex64)

    for i in range(len(sig)):
        h = (np.random.randn() + 1j * np.random.randn()) / np.sqrt(2)
        n = constant_AWGN
        r[i] = h * sig[i] + n
        equalized_signal[i] = (np.conjugate(h) / (np.abs(h) ** 2 + 1 / snr)) * r[i]
...
def load_denoiser_for_snr(snr_db):
    ...
    if not os.path.exists(path):
        raise FileNotFoundError(f"Model for SNR {snr_db} not found.")
...
def get_reconstructed_embedding():
    try:
        ...
        # Check if the noisy_embedding shape is valid
        print(f"Shape of noisy_embedding: {noisy_embedding.shape}")

        # Load the denoiser model for the given SNR
        den = load_denoiser_for_snr(snr)

        # Convert noisy embedding to tensor
        ...
        print(f"Shape of H_den: {H_den.shape}")

        return jsonify(reconstructed_embedding=H_den.tolist())
    except FileNotFoundError as e:
        return jsonify(error=str(e)), 400
    except Exception as e:
        return jsonify(error=f"Error during denoising: {str(e)}"), 500
